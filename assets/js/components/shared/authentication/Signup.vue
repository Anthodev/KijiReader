            ...
            <v-card class="elevation-12">
                ...
                <v-card-text>
                    <v-form id="signupForm" @keyup.native.enter="onSubmit" @submit.prevent="onSubmit">
                        ...
                        <v-progress-linear :value="passwordConfirmLengthProgress" :color="colorPasswordConfirm" height="7"></v-progress-linear>
                    </v-form>
                </v-card-text>
                <v-card-actions>
                    <v-card-text class="caption"><router-link :to="{ name: 'signin' }">Login to your account</router-link></v-card-text>
                    <v-spacer />
                    <v-btn form="signupForm" type="submit" color="primary" :loading="loading" :disabled="$v.$invalid">Login</v-btn>
                </v-card-actions>
            </v-card>
        </v-col>
    </v-row>
...
export default {
    ...
    methods: {
        onSubmit() {
            ...
            this.$store.dispatch('signup', formData).then(this.loading = !this.loading)
        }
    }
}
</script>
